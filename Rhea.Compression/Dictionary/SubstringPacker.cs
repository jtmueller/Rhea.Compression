...
{
    public class SubstringPacker : IDisposable
    {
        ...
        public SubstringPacker(byte[] dictionary) : this(dictionary.AsMemory()) { }

        public SubstringPacker(ReadOnlyMemory<byte> dictionary)
        {
        ...
        public void Pack(Span<byte> bytes, IPackerOutput packerOutput, object? consumerContext)
        {
            using (var hash = new PrefixHash(bytes, false))
            {
                ...
                int curr, count;
                for (curr = 0, count = bytes.Length; curr < count; curr++)
                {
                    ...
                    {
                        PrefixHash.Match match = _dictHash.GetBestMatch(curr, bytes);
                        bestMatchIndex = match.BestMatchIndex;
                        bestMatchLength = match.BestMatchLength;

                        match = hash.GetBestMatch(curr, bytes);
                    ...
                    else if (previousMatchLength > 0 && bestMatchLength > previousMatchLength)
                    {
                        ...
                        packerOutput.EncodeLiteral(bytes[curr - 1], consumerContext);
                    }
                    ...
                    else if (bestMatchLength == 0 && previousMatchLength == 0)
                    {
                        ...
                        packerOutput.EncodeLiteral(bytes[curr], consumerContext);
                    }
                }
            }
